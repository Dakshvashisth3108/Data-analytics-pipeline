...
# multiple mirror URLs, retries each up to 3 times with backoff. If all
# downloads fail, prints clear manual-install instructions.
#
...
# github.com/.../raw/... is a fallback in case raw.githubusercontent is blocked.
$Mirrors = @(
...
if (-not ($ok1 -and $ok2)) {
    ...
    Write-Host "  $BinDir"
    Write-Host ""
    Write-Host "After saving, re-run this script — it'll skip the download and"
